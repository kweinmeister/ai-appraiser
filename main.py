...
LOCATION = os.environ.get("GOOGLE_CLOUD_REGION", "us-central1") # or "us-central1" or your preferred location
...
# Initialize Google Cloud clients globally (for efficiency)
...
# --- FastAPI App ---
app = FastAPI(title="Item Valuation API", description="API to estimate item value based on image and text.")
...
def upload_image_to_gcs(file: UploadFile) -> str:
    ...
    try:
        ...
        print(e)
        raise HTTPException(status_code=500, detail=f"Error uploading image to Cloud Storage: {e}")

def generate_item_description(image_uri: str, description: str) -> str:
    """Calls Gemini API to generate item description, including details helpful for valuation."""
    try:
        mime_type = guess_type(image_uri)[0]
        description_prompt = f"""Describe the item in this image. 
...
User provided information: {description}
"""
        ...
        print(e)
        raise HTTPException(status_code=500, detail=f"Error generating item description: {e}")

def estimate_value_with_search(item_description: str, user_provided_information: str) -> ValuationResponse:
...
Here is a description of the item: {item_description}.
Here is additional information provided by the user: {user_provided_information}.
...
"""
    ...
    try:
        ...
        config_for_parsing = GenerateContentConfig(response_mime_type="application/json", response_schema=ValuationResponse)
...
Ensure the JSON is valid and contains the estimated_value, reasoning, and search_urls fields."""
        ...
        raise HTTPException(status_code=500, detail=f"Error estimating value with search: {e}")
...
async def estimate_item_value(
    ...
    transaction_id = datetime.datetime.utcnow().strftime("%Y%m%d%H%M%S%f") # Simple transaction ID
    ...
    try:
        ...
        print(response_data)
        formatted_value = f"{response_data.estimated_value:.2f}"  # Format to two decimal places
        html_content = f"""
        <div class="bg-white p-4 rounded shadow-md">
            <h2 class="text-xl font-semibold mb-2">Valuation Results</h2>
            <p><strong>Estimated Value:</strong> ${formatted_value}</p>
            <br>
            <p class="text-sm">{response_data.reasoning}</p>"""

        if response_data.search_urls:
            html_content += f"""
            <br>
        ...
        print(e)
        raise HTTPException(status_code=500, detail="Internal server error during valuation.")
